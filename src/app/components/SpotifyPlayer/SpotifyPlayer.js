...
// Lists the account's Spotify devices and moves playback to the one picked
function DevicePicker({ spotify, className }) {
  const [open, setOpen] = useState(false);
  const [devices, setDevices] = useState(null);
  const ref = useRef(null);
  const { listDevices, transferTo } = spotify;
        ...
        <div className="sticker absolute bottom-full right-0 mb-4 w-72 bg-shell rounded-2xl p-2 z-[70]">
          ...
          {devices === null ? (
            ...
            <p className="px-2 pb-2 text-sm text-ink/60">
              No devices found. Open Spotify on your phone or computer and it&apos;ll show up here.
            </p>
          ) : (
            devices.map((d) => (
              <button
...
// Current position, advanced locally between Spotify's state updates
...
export default function SpotifyPlayer({ mode = "Focus Time" }) {
  ...
  const setPauseOnBreaks = useCallback((on) => {
    setPauseOnBreaksState(on);
    try {
  ...
  useEffect(() => {
    ...
    if (!pauseOnBreaks || !isHere) return;
    const onBreak = mode !== "Focus Time";
    if (onBreak && playback && !playback.paused) {
      spotify.pause();
      ...
      <div className="relative w-full h-20 shrink-0 border-t-2 border-ink bg-shell text-ink px-3 md:px-6 grid grid-cols-[minmax(0,1fr)_auto] md:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_minmax(0,1fr)] items-center gap-3 md:gap-6">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            {subtitle ? <p className="text-sm text-ink/60 truncate">{subtitle}</p> : null}
          </div>
          {saved !== null && track ? (
            <button
              type="button"
              onClick={spotify.toggleSaved}
        ...
        <div className="flex flex-col items-center justify-center gap-1 md:min-w-0">
          {elsewhere && !isHere ? (
            ...
            <div className="flex items-center gap-1 md:gap-2">
              ...
              <button type="button" onClick={spotify.previous} disabled={!canControl} aria-label="Previous track" className={`${iconBtn} hidden sm:flex`}>
                <Icon d={PREV} filled />
              </button>
              <button
                type="button"
              ...
              <button type="button" onClick={spotify.next} disabled={!canControl} aria-label="Next track" className={`${iconBtn} flex`}>
                <Icon d={NEXT} filled />
              </button>
              <button
                type="button"
              ...
              <button
                ...
                aria-label={error?.canReconnect ? "Reconnect Spotify" : "Browse music"}
        ...
        <div className="hidden md:flex items-center justify-end gap-3">
          {status === "ready" ? (
            <DevicePicker spotify={spotify} className={`${iconBtn} flex`} />
          ) : null}
          <button
            type="button"
            onClick={() => {
              if (volume > 0) {
                ...
                spotify.setVolume(0);
              } else spotify.setVolume(lastVolume || 0.5);
            }}
          ...
          <Slider value={volume} max={1} label="Volume" onChange={spotify.setVolume} className="w-24" />
